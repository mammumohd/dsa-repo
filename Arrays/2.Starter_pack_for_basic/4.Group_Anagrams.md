...
var groupAnagrams = function(strs) {
    let anagramCounterMap = new Map();

    for(let index=0; index < strs.length; index++) {
       let keystring = strs[index].split("").sort().join("");
       
       if (!anagramCounterMap.has(keystring)){
        anagramCounterMap.set(keystring,[])
       }

       anagramCounterMap.get(keystring).push(strs[index])
    }

    return Array.from(anagramCounterMap.values())
};
